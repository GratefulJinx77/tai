...
- Create screenshot directory: `mkdir -p /tmp/tai-browser/<story-slug>/`

### Phase 2: Setup Session
```bash
mkdir -p /tmp/tai-browser/<story-slug>/
...
playwright-cli -s=review-<story-slug> screenshot --filename=/tmp/tai-browser/<story-slug>/00_initial.png
```
...
|----------------|-------------|
| `snapshot_contains` | `playwright-cli snapshot` then search output for text |
| `url_matches` | `playwright-cli eval "window.location.href"` then match pattern |
| `element_visible` | `playwright-cli snapshot` then element ref exists |
| `element_absent` | `playwright-cli snapshot` then element ref NOT found |
| `console_clean` | `playwright-cli console` then no errors |
| `visual_match` | `playwright-cli screenshot` then compare (requires human review) |
...
```json
{
  ...
  "steps": [
    {
      ...
      "ref": "e12",
      "result": "SUCCESS",
      "screenshot": "/tmp/tai-browser/<slug>/01_filled-email.png"
    }
  ],
  "assertions": [
    {
  ...
  "screenshots": ["/tmp/tai-browser/<slug>/00_initial.png", "..."],
  "errors": [],
  "duration_seconds": 12
}
```
...
You have a persistent memory directory at `.tai/agent-memory/UIReviewer/`. Its contents persist across conversations.

**MEMORY.md** is loaded into your system prompt automatically (lines after 200 are truncated — keep it concise).

**What to save:** Stable patterns, recurring UI issues, file paths, debugging insights.
