 ...
 *
 * Configuration files:
 *   Base:  .tai/skills/Agents/Data/Traits.yaml
 *   User:  .tai/USER/SKILLCUSTOMIZATIONS/Agents/Traits.yaml
 *
 * Usage:
...
// Paths - resolve relative to .tai/ directory
import { join, dirname } from "path";

function findTaiDir(): string {
  let dir = process.cwd();
  while (dir !== '/') {
...
const TAI_DIR = process.env.TAI_DIR || findTaiDir();
const BASE_TRAITS_PATH = join(TAI_DIR, 'skills', 'agents', 'Data', 'Traits.yaml');
const USER_TRAITS_PATH = join(TAI_DIR, 'skills', 'agents', 'Data', 'UserTraits.yaml');
const TEMPLATE_PATH = join(TAI_DIR, 'skills', 'agents', 'Templates', 'DynamicAgent.hbs');
const CUSTOM_AGENTS_DIR = join(TAI_DIR, 'agents', 'custom');
...
/**
 * Save a composed agent to .tai/custom-agents/{slug}.md
 *
 * Produces a CLAUDE CODE COMPATIBLE agent file that can be:
 * 1. Copied to .tai/agents/ and used as a built-in agent
 ...
 *
 * Matches the structural format of built-in agents in .tai/agents/*.md:
...
\`\`\`bash
bun run .tai/skills/Agents/Tools/ComposeAgent.ts --load "${slug}"
\`\`\`

Or reconstruct from traits:

\`\`\`bash
bun run .tai/skills/Agents/Tools/ComposeAgent.ts --traits "${agent.traits.join(",")}"
\`\`\`
...
OPTIONS:
  ...
  -s, --save           Save composed agent to .tai/custom-agents/
...
CONFIGURATION:
  Base traits:    .tai/skills/Agents/Data/Traits.yaml
  User traits:    .tai/USER/SKILLCUSTOMIZATIONS/Agents/Traits.yaml
  Custom agents:  .tai/custom-agents/
