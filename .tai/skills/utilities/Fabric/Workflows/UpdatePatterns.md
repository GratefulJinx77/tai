...
```bash
CURRENT_COUNT=$(ls -1 .tai/skills/utilities/Fabric/Patterns/ 2>/dev/null | wc -l | tr -d ' ')
...
```bash
rsync -av --delete ~/.config/fabric/patterns/ .tai/skills/utilities/Fabric/Patterns/
```

### Step 5: Report Results

```bash
NEW_COUNT=$(ls -1 .tai/skills/utilities/Fabric/Patterns/ 2>/dev/null | wc -l | tr -d ' ')
echo ""
...
for pattern in extract_wisdom summarize create_threat_model analyze_claims; do
  if [ -d .tai/skills/utilities/Fabric/Patterns/$pattern ]; then
    echo "✓ $pattern"
  else
...
# Sync patterns
rsync -av --delete patterns/ .tai/skills/utilities/Fabric/Patterns/
...
```bash
# Count patterns
ls -1 .tai/skills/utilities/Fabric/Patterns/ | wc -l

# List recent additions (if patterns have dates)
ls -lt .tai/skills/utilities/Fabric/Patterns/ | head -10
```
