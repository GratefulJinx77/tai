...
```bash
.tai/skills/[skill-name]/SKILL.md
```
...
```bash
cp -r .tai/skills/[skill-name]/ .tai/memory/history/Backups/[skill-name]-backup-$(date +%Y%m%d)/
```

**Note:** Backups go to `.tai/memory/history/Backups/`, NEVER inside skill directories.
...
cd .tai/skills/[SkillName]/Workflows/
...
find .tai/skills/[SkillName]/ -type d -mindepth 2 -maxdepth 3
```
