...
WORK_DIR=$(jq -r '.work_dir' .tai/memory/state/current-work.json)

# Example: Recent nmap scan
SCAN_FILE=.tai/memory/state/work/${WORK_DIR}/$(date +%Y-%m-%d)_nmap-scan.txt

# Or masscan results
MASSCAN_FILE=.tai/memory/state/work/${WORK_DIR}/$(date +%Y-%m-%d)_masscan.json

# Or combined multi-tool archive from history
SCAN_DIR=.tai/memory/history/security/scans/2025-11-target-recon/
```
...
// Usage: bun run tools/batch-scan-analysis.ts .tai/memory/state/work/{current_work}/target-scans/
```
