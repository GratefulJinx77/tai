...
Environment Variables (add to ${TAI_DIR}/.env):
...
try:
    ...
    load_dotenv(os.path.expanduser(os.environ.get("TAI_DIR", ".tai") + "/.env"))
except ImportError:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        if not self.api_key:
            raise APIKeyMissingError(
                "Shodan API key not found. Set SHODAN_API_KEY in ${TAI_DIR}/.env",
                service="Shodan"
            )
...
class DehashedClient:
    ...
    def __init__(self, api_key: Optional[str] = None, email: Optional[str] = None):
        ...
        if not self.api_key:
            raise APIKeyMissingError(
                "Dehashed API key not found. Set DEHASHED_API_KEY in ${TAI_DIR}/.env",
                service="Dehashed"
            )

        if not self.email:
            raise APIKeyMissingError(
                "Dehashed email not found. Set DEHASHED_EMAIL in ${TAI_DIR}/.env",
                service="Dehashed"
            )
...
class OSINTIndustriesClient:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        if not self.api_key:
            raise APIKeyMissingError(
                "OSINT Industries API key not found. Set OSINT_INDUSTRIES_API_KEY in ${TAI_DIR}/.env",
