# TAI PRD Format Specification v2.0

The PRD (Product Requirements Document) is the single source of truth for every work item.
The AI writes all PRD content directly using Write/Edit tools.
...
- `slug`: Format `YYYYMMDD-HHMMSS_kebab-description`. Used as directory name under `memory/state/work/`.
- `effort`: Determines ISC count range and time budget.
- `phase`: Updated at the START of each phase. Set to `complete` when done.
- `progress`: Format `M/N` where M = checked ISC criteria, N = total ISC criteria. Updated immediately when a criterion passes.
- `mode`: `interactive` (single run) or `loop` (multiple iterations toward ideal state).
...
- Check (`- [x]`) immediately when satisfied — don't batch
...
**Count enforcement:** Total ISC must meet effort tier floor (Standard: 8, Extended: 16, Advanced: 24, Deep: 40, Comprehensive: 64).
...
- ISC-2: `npm test` passes, 14/14 tests green
...
```
.tai/memory/state/work/{slug}/PRD.md
```

Directory created with `mkdir -p memory/state/work/{slug}/` during OBSERVE.

## Continuation / Rework

When a follow-up prompt continues the same task:

1. Detect recent PRD matching the task context
2. Edit existing PRD: reset `phase: observe`, add/increment `iteration`, update `updated`
3. Re-enter phases as needed
...
Google Design Docs, Amazon 6-pagers, Shape Up pitches, and production PRD usage.

Key design choices:
- **8 fields, not 15**: Only fields consumed by the workflow. Dead fields waste tokens.
- **4 sections, not 7**: Risks merged into Context. Plan merged into Context. Changelog dropped (git serves this purpose).
- **Checkboxes over EARS/BDD**: Simpler to parse, write, and verify. ISC pattern proven in production.
