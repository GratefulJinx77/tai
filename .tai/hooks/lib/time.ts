 ...
 *
 * Consistent timestamp generation across the TAI hook system.
 * Reads timezone from team.yaml via getTeamTimezone()
 */

import { getTeamTimezone } from './identity';

/**
 * Get configured timezone from team.yaml (defaults to UTC)
 */
function getTimezone(): string {
  return getTeamTimezone() || 'UTC';
}
...
export function getYearMonth(): string {
  return getDateString().substring(0, 7);
}
