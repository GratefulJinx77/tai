...
#
# NOTE: Wrapping in main() ensures bash reads the entire script before executing.
# This is critical for `curl | bash` — without it, bash reads stdin line-by-line
# and the interactive prompts compete with the pipe for stdin.
# ═══════════════════════════════════════════════════════════════════════════════
...
main() {
    ...
    # ── Reconnect stdin to TTY for interactive prompts ───────────────────────
    # When run via `curl | bash`, stdin is the pipe. The wizard needs the
    # terminal for user input, so we redirect from /dev/tty.
